...
from .methodology import METHODOLOGY_VERSIONS
    ...
    return {
        ...
        "data_dictionary": {
            ...
            "predictions": (
                ...
                "sai, các chỉ số trùng một phần, baseline theo sản phẩm và so sánh cặp "
                "từng chiến lược với baseline."
            ),
            "audit_summary": (
        ...
        "methodology": {
            ...
            "backtest": {
                ...
                "paired_permutation_test_field": (
                    "product_reports[*].backtest.*.paired_permutation_test"
                ),
                "effect_summary_field": (
                    "product_reports[*].backtest.*.effect_summary"
                ),
                "strategy_pairwise_comparison_field": (
                    "product_reports[*].backtest.strategy_pairwise_comparisons"
                ),
                ...
                "strategy_pairwise_validation": manifest.get(
                    "backtest_summary",
                    {},
                ).get("strategy_pairwise_validation", {}),
            ...
            "predictions": {
                "baseline_interval_field": (
                    "predictions.outcome_summary.baseline_intervals"
                ),
                "product_baseline_field": "predictions.baseline_summary.products",
                "strategy_baseline_comparison_field": (
                    "predictions.strategy_baseline_comparisons"
                ),
                "prediction_interval_method": "poisson_binomial_exact_dp",
                "paired_comparison": (
                    "observed_score_minus_same_draw_uniform_expected_score"
                ),
                "pooled_rate_policy": (
                    "do_not_publish_pooled_hit_rates_across_different_outcome_spaces"
                ),
                "minimum_evaluations_for_baseline_claim": prediction_report.get(
                    "baseline_summary",
                    {},
                ).get("minimum_evaluations_for_baseline_claim"),
            },
            "fairness_audit": {
