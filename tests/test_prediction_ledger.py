...
def test_walk_forward_backtest_reports_uniform_baseline() -> None:
    ...
    permutation = report["comparison"]["paired_permutation_test"]
    assert permutation["status"] == "available"
    assert permutation["method"] == "paired_sign_flip_monte_carlo"
    assert permutation["resamples"] == predictions_module.BACKTEST_PAIRED_PERMUTATION_RESAMPLES
    assert 0 <= permutation["p_value"] <= 1
    assert permutation["no_multiple_testing_decision"] is True
    effect = report["comparison"]["effect_summary"]
    assert effect["absolute_effect"] == report["comparison"]["mean_hit_difference"]
    assert effect["practical_effect_threshold"] == (
        predictions_module.BACKTEST_PRACTICAL_EFFECT_THRESHOLD
    )
    pairwise = report["strategy_pairwise_comparisons"]
    assert pairwise["method"] == "paired_strategy_score_differences"
    assert pairwise["comparison_count"] == 3
    assert pairwise["target_scope_id"] == target_scope["scope_id"]
    assert {
        row["comparison_id"] for row in pairwise["comparisons"]
    } == {
...
def test_digit_walk_forward_backtest_reports_digit_score_formula() -> None:
    ...
    assert permutation["method"] == "paired_sign_flip_monte_carlo"
    assert 0 <= permutation["p_value"] <= 1
    assert report["comparison"]["effect_summary"]["absolute_effect"] == report[
        "comparison"
    ]["mean_position_match_difference"]
    pairwise = report["strategy_pairwise_comparisons"]
    assert pairwise["method"] == "paired_strategy_score_differences"
    assert pairwise["comparison_count"] == 3
    assert pairwise["score_unit"] == "best_position_matches_per_draw"
    assert all(
        row["target_scope_id"] == report["target_scope"]["scope_id"]
        and row["paired_permutation_test"]["status"] == "available"
        for row in pairwise["comparisons"]
    )
    trial_log = report["trial_disposition_log"]
...
def _mock_comparison_diagnostics(
    *,
    scope_id: str,
    final_count: int,
    difference: float,
) -> dict[str, object]:
    threshold = predictions_module.BACKTEST_PRACTICAL_EFFECT_THRESHOLD
    ...
    return {
        ...
        "paired_permutation_test": {
            ...
            "observed_statistic": difference,
            "p_value": 1.0,
            "resamples": predictions_module.BACKTEST_PAIRED_PERMUTATION_RESAMPLES,
            "seed": "0" * 16,
            "values_sha256": "0" * 64,
            "full_value_count": final_count,
            "permutation_value_count": final_count,
            "sampling_method": "full_sequence",
        ...
        "effect_summary": {
            ...
            "comparator_average": None,
            "practical_effect_threshold": threshold,
            "practical_effect_unit": "score_units_per_draw",
            "practically_meaningful": abs(difference) >= threshold,
            "practical_significance": practical_significance,
        },
    }


def _mock_strategy_pairwise_comparisons(
    *,
    scope_id: str,
    final_count: int,
) -> dict[str, object]:
    ...
    rows = [
        {
            ...
            "strategy_a": left,
            "strategy_b": right,
            "strategy_a_label": left,
            "strategy_b_label": right,
            "score_unit": "score_units_per_draw",
            "mean_strategy_a_score": 0.0,
            "mean_strategy_b_score": 0.0,
            "mean_score_difference": 0.0,
            "paired_z_score": 0.0,
            "approximate_p_value": 1.0,
            **_mock_comparison_diagnostics(
                scope_id=scope_id,
    ...
    return {
        ...
        "permutation_resamples": (
            predictions_module.BACKTEST_PAIRED_PERMUTATION_RESAMPLES
        ),
        "practical_effect_threshold": (
            predictions_module.BACKTEST_PRACTICAL_EFFECT_THRESHOLD
        ),
        "practically_meaningful_count": 0,
        "comparisons": rows,
    }
...
def test_finalize_backtests_applies_global_bh_correction() -> None:
    ...
    reports = [
        {
            ...
            "backtest": {
                ...
                "strategy_pairwise_comparisons": _mock_strategy_pairwise_comparisons(
                    scope_id="scope-a",
                    final_count=10,
                ),
            },
    ...
    assert summary["strategy_pairwise_validation"]["status"] == "validated"
    assert summary["strategy_pairwise_validation"]["comparison_count"] == 6
...
def test_prediction_report_uses_strict_exact_and_near_rules(tmp_path) -> None:
    ...
    assert summary["pooled_rate_suppressed"] is True
    assert summary["minimum_evaluations_for_baseline_claim"] == (
        predictions_module.PREDICTION_BASELINE_MIN_EVALUATIONS_FOR_CLAIM
    )
    assert summary["baseline_intervals"]["exact"]["method"] == (
        "poisson_binomial_exact_dp"
    )
    assert summary["baseline_intervals"]["near"]["trials"] == 3
    assert summary["baseline_intervals"]["near"]["expected_count"] > 0
    assert summary["expected_score_by_chance"] > 0
    assert summary["score_excess_vs_chance"] > 0
    ...
    assert product_outcome["score_basis"] == "main_numbers"
    assert product_outcome["expected_near_by_chance"] == summary["expected_near_by_chance"]
    assert product_outcome["baseline_intervals"]["near"] == summary[
        "baseline_intervals"
    ]["near"]
    assert product_outcome["baseline_summary"]["baseline_model"] == (
        "uniform_same_ticket_shape"
    )
    ...
    assert near_row["outcome"]["baseline_probability"]["expected_score"] > 0

    baseline_summary = report["baseline_summary"]
    assert baseline_summary["pooled_rate_suppressed"] is True
    assert baseline_summary["products"]["mega645"]["score_basis"] == "main_numbers"
    [comparison] = report["strategy_baseline_comparisons"]
    assert comparison["product"] == "mega645"
    assert comparison["strategy"] == "balanced_signal"
    assert comparison["comparison_method"] == (
        "paired_observed_score_minus_same_draw_uniform_expectation"
    )
    assert comparison["paired_permutation_test"]["status"] == "available"
    assert comparison["sample_size_status"] == "insufficient"
    assert comparison["claim_status"] == "blocked_minimum_sample"
    assert comparison["beats_baseline_claim_allowed"] is False
...
def test_digit_near_probability_accounts_for_multiple_prize_outcomes(tmp_path) -> None:
    ...
    assert baseline["expected_score"] > 0
    assert report["product_outcomes"]["max3dpro"]["expected_near_by_chance"] == 0.401
    assert report["product_outcomes"]["max3dpro"]["score_basis"] == (
        "best_position_matches"
    )
