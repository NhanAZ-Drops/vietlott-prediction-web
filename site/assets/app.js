...
function renderBacktest(backtest, kind) {
  ...
  container.innerHTML = `
    ...
    ${modelRows.map((row) => {
      ...
      const permutation = row.comparison.paired_permutation_test || {};
      const effect = row.comparison.effect_summary || {};
      const relativeEffect = effect.relative_effect_percent == null
        ? "N/A"
        : `${formatSigned(effect.relative_effect_percent, 1)}%`;
      return `
        <p class="backtest-evidence">
          ...
          ${numberFormatter.format(backtest.samples)} kỳ kiểm tra. Permutation p
          ${formatPValue(permutation.p_value)}; hiệu ứng tuyệt đối
          ${formatSigned(effect.absolute_effect ?? comparisonValue)}, tương đối
          ${escapeHtml(relativeEffect)}.
        </p>`;
    }).join("")}
    ${renderBacktestPairwise(backtest.strategy_pairwise_comparisons)}
    ...
    <details class="backtest-method-details">
      ...
      <div class="backtest-method-body">
        <ol>
          ...
          ${renderBacktestPairwiseMethod(backtest.strategy_pairwise_comparisons)}
...
function renderBacktestPairwise(pairwise) {
  if (!pairwise || !Array.isArray(pairwise.comparisons)) return "";
  const rows = pairwise.comparisons.map((row) => {
    const permutation = row.paired_permutation_test || {};
    const effect = row.effect_summary || {};
    const relativeEffect = effect.relative_effect_percent == null
      ? "N/A"
      : `${formatSigned(effect.relative_effect_percent, 1)}%`;
    return `
      <div class="backtest-pairwise-row">
        <span>${escapeHtml(backtestStrategyLabel(row.strategy_a))} - ${escapeHtml(backtestStrategyLabel(row.strategy_b))}</span>
        <strong>${formatSigned(row.mean_score_difference)}</strong>
        <small>
          95% [${formatSigned(row.confidence_interval_lower)}, ${formatSigned(row.confidence_interval_upper)}],
          permutation p ${formatPValue(permutation.p_value)},
          hiệu ứng tương đối ${escapeHtml(relativeEffect)}
        </small>
      </div>`;
  }).join("");
  return `
    <section class="backtest-pairwise" aria-label="So sánh cặp chiến lược">
      <div class="backtest-pairwise-heading">
        <span>So sánh cặp chiến lược</span>
        <small>${numberFormatter.format(pairwise.comparison_count || 0)} cặp trên cùng target scope</small>
      </div>
      <div class="backtest-pairwise-grid">${rows}</div>
    </section>`;
}

function renderBacktestPairwiseMethod(pairwise) {
  if (!pairwise || !Array.isArray(pairwise.comparisons)) return "";
  return `
    <li><strong>So sánh trực tiếp giữa chiến lược</strong><span>
      Ba chiến lược công bố được so từng cặp trên cùng target_scope bằng chênh lệch điểm theo kỳ,
      khoảng tin cậy 95% và paired sign-flip permutation. Phần này là chẩn đoán phụ, không thay
      p/q/status của so sánh với baseline đồng đều.
    </span></li>`;
}

function backtestStrategyLabel(strategy) {
  const labels = {
    balanced_signal: "Kết hợp ba dấu hiệu",
    recent_frequency: "Tần suất cửa sổ gần",
    audit_signal: "Tín hiệu kiểm định",
  };
  return labels[strategy] || strategy || "Chiến lược";
}
...
function renderPredictionShell(predictions, products) {
  ...
  const nearInterval = outcome.baseline_intervals?.near;
  const minimumForClaim = Number(outcome.minimum_evaluations_for_baseline_claim || 0);
  const nearIntervalText = nearInterval
    ? ` Khoảng dự đoán 95% dưới nền: ${predictionIntervalLabel(nearInterval)} lượt.`
    : "";
  const claimGuard = minimumForClaim
    ? ` Mỗi chiến lược cần ít nhất ${numberFormatter.format(minimumForClaim)} lượt đã chấm trước khi công bố lợi thế so với nền.`
    : "";
...
function renderPredictionResults(slug) {
  ...
  const expectedExact = Number(productOutcome.expected_exact_by_chance || 0);
  ...
  const scoreExcess = Number(productOutcome.score_excess_vs_chance || 0);
  ...
  const baselineComparisons = (state.predictions.strategy_baseline_comparisons || [])
    .filter((comparison) => comparison.product === slug);
  ...
  summary.innerHTML = `
    <div class="prediction-product-metrics">
      ...
      <div class="prediction-product-metric static"><span>Kỳ vọng đúng</span><strong>${formatExpectedCount(expectedExact)}</strong></div>
      ...
      <div class="prediction-product-metric static"><span>Chênh điểm</span><strong>${formatSignedExpected(scoreExcess)}</strong></div>
    ...
    ${renderPredictionBaselinePanel(productOutcome, baselineComparisons)}`;
...
function renderPredictionBaselinePanel(productOutcome, comparisons) {
  const intervals = productOutcome.baseline_intervals || {};
  const baseline = productOutcome.baseline_summary || {};
  const scoreLabel = productOutcome.score_kind === "positions"
    ? "Điểm vị trí"
    : "Điểm số chính";
  const rows = [
    {
      label: "Đúng toàn bộ",
      observed: productOutcome.exact,
      expected: productOutcome.expected_exact_by_chance,
      interval: intervals.exact,
    },
    {
      label: "Gần đúng",
      observed: productOutcome.near,
      expected: productOutcome.expected_near_by_chance,
      interval: intervals.near,
    },
    {
      label: scoreLabel,
      observed: baseline.observed_score_sum,
      expected: baseline.expected_score_by_chance,
      interval: null,
    },
  ];
  const comparisonRows = comparisons.length
    ? comparisons
      .map((comparison) => {
        const copy = predictionStrategyCopy(comparison.strategy);
        const status = comparison.beats_baseline_claim_allowed
          ? "Đủ điều kiện công bố lợi thế"
          : comparison.sample_size_status === "insufficient"
            ? `Chưa đủ ${numberFormatter.format(comparison.minimum_evaluations_for_claim || 0)} lượt`
            : "Chưa vượt nền";
        return `
          <div class="prediction-baseline-strategy">
            <span>${escapeHtml(copy.title)}</span>
            <strong>${formatSignedExpected(comparison.mean_score_difference)}</strong>
            <small>
              p ${formatPValue(comparison.paired_p_value)}
              · ${numberFormatter.format(comparison.evaluations || 0)} lượt · ${escapeHtml(status)}
            </small>
          </div>`;
      })
      .join("")
    : '<div class="prediction-empty">Chưa có so sánh chiến lược cho sản phẩm này.</div>';

  return `
    <div class="prediction-baseline-panel">
      <div class="prediction-baseline-heading">
        <span>Baseline theo sản phẩm</span>
        <p>Không gộp tỷ lệ giữa các trò chơi có không gian kết quả khác nhau.</p>
      </div>
      <div class="prediction-baseline-grid">
        ${rows
          .map((row) => `
            <div>
              <span>${escapeHtml(row.label)}</span>
              <strong>${formatExpectedCount(row.observed || 0)} / ${formatExpectedCount(row.expected || 0)}</strong>
              <small>${row.interval ? `95%: ${predictionIntervalLabel(row.interval)} lượt` : "Quan sát / kỳ vọng"}</small>
            </div>`)
          .join("")}
      </div>
      <div class="prediction-baseline-strategies">
        ${comparisonRows}
      </div>
    </div>`;
}
...
function predictionOutcomeConclusion() {
  ...
  const minimum = Number(summary.minimum_evaluations_for_baseline_claim || 0);
  const comparisons = state.predictions?.strategy_baseline_comparisons || [];
  const allowedClaims = comparisons.filter((comparison) => comparison.beats_baseline_claim_allowed);
  const baselineGuard = allowedClaims.length
    ? `${allowedClaims.length} chiến lược đã qua ngưỡng mẫu và kiểm định paired so với nền.`
    : minimum
      ? `Chưa có chiến lược nào đủ ngưỡng ${numberFormatter.format(minimum)} lượt và kiểm định paired để công bố lợi thế so với nền.`
      : "Chưa có chiến lược nào đủ điều kiện công bố lợi thế so với nền.";
  if (summary.exact) {
    return `Sổ dự đoán ngoài mẫu hiện có ${summary.exact} lượt đúng toàn bộ, ${summary.near} lượt gần đúng và ${summary.wrong} lượt sai. ${baselineGuard}`;
  }
  return `Sau ${state.predictions.evaluation_count} lượt đã đối chiếu, chưa có dự đoán nào đúng toàn bộ hoặc gần đúng theo tiêu chí nghiêm; ${summary.partial_matches} lượt có trùng một phần. ${baselineGuard}`;
}
...
function predictionIntervalLabel(interval) {
  if (!interval) return "chưa có";
  return `${numberFormatter.format(interval.lower_count || 0)}-${numberFormatter.format(interval.upper_count || 0)}`;
}
