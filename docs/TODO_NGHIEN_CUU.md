...
- [~] `DATA-001` Viết định nghĩa chính xác cho `confirmed`, `not_confirmed`,
  ...
  - Đã triển khai cục bộ ngày 15/06/2026. Bằng chứng `src/vietlott_analytics/provenance.py`,
    `tests/test_analytics_io.py` và `docs/THU_THAP_DU_LIEU.md`. Chờ CI.
...
- [x] `BACKTEST-010` So sánh xấp xỉ chuẩn hiện tại với block bootstrap.
  ...
  - Website hiển thị "Block bootstrap theo chuỗi"; tài liệu hóa trong
    ...
    test, static-site test.
- [x] `BACKTEST-011` Thêm khoảng tin cậy cho chênh lệch giữa hai chiến lược trên cùng kỳ.
  - Hoàn thành ngày 19/06/2026. Bằng chứng `src/vietlott_analytics/predictions.py`,
    `site/data/products/*.json`, `site/assets/app.js`, `tests/test_prediction_ledger.py`
    và `tests/test_static_site.py`.
- [x] `BACKTEST-012` Thêm kiểm định paired permutation hoặc phương pháp tương đương.
  - Hoàn thành ngày 19/06/2026. Bằng chứng `paired_permutation_test` trong các
    comparison công bố và `strategy_pairwise_comparisons`, kiểm chứng bằng `pytest`.
- [x] `BACKTEST-013` Báo cáo hiệu ứng tuyệt đối, hiệu ứng tương đối và ý nghĩa thực dụng.
  - Hoàn thành ngày 19/06/2026. Bằng chứng `effect_summary`,
    `BACKTEST_SCORE_FORMULAS.md`, `phuong-phap.html` và artifact website mới.
...
- [x] `PRED-002` Tính xác suất nền riêng cho mỗi sản phẩm và mỗi loại điểm.
  - `outcome.baseline_probability` hiện công bố `exact`, `near`,
    `matched_units`, `score`, `expected_score`, `score_basis` theo từng lượt đã
    chấm. `product_outcomes.<slug>` và `baseline_summary.products.<slug>` giữ
    riêng từng sản phẩm/kiểu điểm. Hoàn thành ngày 19/06/2026.
- [x] `PRED-003` Tính số lần đúng kỳ vọng và khoảng dự đoán dưới baseline.
  - `baseline_intervals.exact` và `baseline_intervals.near` dùng
    `poisson_binomial_exact_dp` để công bố kỳ vọng và khoảng 95% dưới baseline.
    Bằng chứng `src/vietlott_analytics/predictions.py`,
    `site/data/predictions.json` và `tests/test_prediction_ledger.py`.
- [x] `PRED-004` Không cộng tỷ lệ đúng của các trò chơi có không gian kết quả khác nhau.
  - `outcome_summary.pooled_rate_suppressed = true`; website chỉ cộng số lượt
    kỳ vọng và hiển thị bảng theo sản phẩm, không công bố tỷ lệ pooled giữa các
    không gian kết quả khác nhau. Bằng chứng `site/assets/app.js`,
    `docs/DATA_DICTIONARY.md` và `tests/test_static_site.py`.
- [x] `PRED-005` So sánh chiến lược với baseline theo từng kỳ bằng phép so sánh cặp.
  - `strategy_baseline_comparisons` so sánh
    `observed_score - expected_score` trên cùng kỳ đích, kèm p xấp xỉ chuẩn,
    permutation p-value và trạng thái claim. Bằng chứng
    `src/vietlott_analytics/predictions.py` và `tests/test_prediction_ledger.py`.
...
- [x] `PRED-011` Đặt cỡ mẫu tối thiểu trước khi website dùng từ "tốt hơn baseline".
  - `PREDICTION_BASELINE_MIN_EVALUATIONS_FOR_CLAIM = 30`; website chỉ công bố
    lợi thế so với nền khi chiến lược đủ số lượt và vượt kiểm định paired. Khi
    chưa đủ điều kiện, giao diện chỉ ghi nhận kết quả quan sát và trạng thái
    "chưa đủ lượt". Bằng chứng `site/assets/app.js`,
    `src/vietlott_analytics/predictions.py` và `tests/test_static_site.py`.
...
- [x] `WEB-008` Hiển thị baseline kỳ vọng cạnh kết quả dự đoán thực tế.
  - Bảng dự đoán theo sản phẩm hiện hiển thị quan sát/kỳ vọng cho đúng toàn bộ,
    gần đúng, điểm số và khoảng 95%; từng chiến lược có chênh điểm paired,
    p-value và trạng thái cỡ mẫu. Bằng chứng `site/assets/app.js`,
    `site/assets/styles.css`, `site/data/predictions.json` và
